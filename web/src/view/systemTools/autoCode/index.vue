  ...
  <div>
        ...
        <el-collapse-item name="1">
          ...
          <el-form ref="getTableForm" style="margin-top:24px" :inline="true" :model="dbform" label-width="120px">
            <el-form-item label="业务库" prop="selectDBtype">
              ...
              <el-select v-model="dbform.businessDB" clearable placeholder="选择业务库" @change="getDbFunc">
    ...
    <div class="gva-search-box">
      ...
      <el-form ref="autoCodeForm" :rules="rules" :model="form" label-width="120px" :inline="true">
        ...
        <el-form-item label="业务库" prop="businessDB">
          ...
          <el-select v-model="form.businessDB" placeholder="选择业务库">
            <el-option v-for="item in dbList" :key="item.aliasName" :value="item.aliasName" :label="item.aliasName"
              :disabled="item.disable">
              <div>
                <span>{{ item.aliasName }}</span>
        ...
        <div>
          ...
          <el-form-item>
            <template #label>
              <el-tooltip content="注：把自动生成的菜单注册进数据库" placement="bottom" effect="light">
                <div> 自动创建菜单 <el-icon>
                    <QuestionFilled />
                  </el-icon></div>
              </el-tooltip>
          ...
          <el-form-item>
            <template #label>
              <el-tooltip content="注：自动同步数据库表结构，如果不需要可以选择关闭。" placement="bottom" effect="light">
                <div> 同步表结构 <el-icon>
                    <QuestionFilled />
                  </el-icon></div>
              </el-tooltip>
    ...
    <div class="gva-table-box">
      ...
      <div class="draggable">
        <el-table :data="form.fields" row-key="fieldName">
          ...
          <el-table-column align="left" prop="front" label="前端可见">
            <template #default="{ row }">
          ...
          <el-table-column align="left" prop="fieldIndexType" label="索引类型" width="160">
            <template #default="{ row }">
              <el-select v-model="row.fieldIndexType" style="width:100%" placeholder="请选择字段索引类型" clearable>
          ...
          <el-table-column align="left" label="操作" width="300" fixed="right">
            <template #default="scope">
              <el-button type="primary" link icon="edit" @click="editAndAddField(scope.row)">
                高级编辑
              </el-button>
      ...
      <div class="gva-btn-list justify-end mt-4">
        <el-button type="primary" @click="clearCatch()">
          清除暂存
        </el-button>
        <el-button type="primary" @click="catchData()">
          暂存
        </el-button>
        <el-button type="primary" @click="enterForm(true)">
          预览代码
        </el-button>
        <el-button type="primary" @click="enterForm(false)">
          生成代码
        </el-button>
      </div>
    </div>
    ...
    <el-drawer v-model="dialogFlag" size="70%" :show-close="false">
      ...
      <FieldDialog v-if="dialogFlag" ref="fieldDialogNode" :dialog-middle="dialogMiddle" :type-options="typeOptions"
        :type-search-options="typeSearchOptions" :type-index-options="typeIndexOptions" />
    </el-drawer>

    <el-drawer v-model="previewFlag" size="60%" :show-close="false">
      <template #header>
        <div class="flex justify-between items-center">
